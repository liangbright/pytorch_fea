...
from torch_fea.element.SurfaceElement_tri3 import cal_dh_du_and_dh_dv, get_integration_point, get_shape_integration_weight
#%% force_1i = force_3i
def cal_nodal_force_from_pressure(pressure, x, n_integration_points=1):
...
if __name__ == "__main__":
    ...
    x=torch.rand(2, 3, 3, device=device_gpu)
    #%%
    import time
    pressure=torch.tensor(16.0, device=device_gpu)
    t0=time.time()
    force_1i = cal_nodal_force_from_pressure(pressure, x, 1)
    force_3i = cal_nodal_force_from_pressure(pressure, x, 3)
    t1=time.time()
