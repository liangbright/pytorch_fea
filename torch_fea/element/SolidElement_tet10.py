# C3D10 in Abaqus, TET10 in FEBio
...
def get_integration_point(n_points, dtype, device):
    if n_points==1:
        ...
        raise ValueError("n_points must be 1 or 4 or 8")
#%% gaussian integration weight
...
#%%
def sf4(r):
    return 4*(1-r[0]-r[1]-r[2])*r[0]
#%%
def cal_d_sf4_dr(r):
    dr0=-8*r[0]+4*(1-r[1]-r[2])
    dr1=-4*r[0]
    dr2=-4*r[0]
    dr=[dr0, dr1, dr2]
    return dr
...
#%%
def sf6(r):
    return 4*(1-r[0]-r[1]-r[2])*r[1]
#%%
def cal_d_sf6_dr(r):
    dr0=-4*r[1]
    dr1=-8*r[1]+4*(1-r[0]-r[2])
    dr2=-4*r[1]
    dr=[dr0, dr1, dr2]
    return dr
#%%
def sf7(r):
    return 4*(1-r[0]-r[1]-r[2])*r[2]
#%%
def cal_d_sf7_dr(r):
    dr0=-4*r[2]
    dr1=-4*r[2]
    dr2=-8*r[2]+4*(1-r[0]-r[1])
    dr=[dr0, dr1, dr2]
    return dr
#%%
def sf8(r):
...
def cal_d_sf_dr(r):
    ...
    d_sf8_dr=torch.cat(cal_d_sf8_dr(r), dim=1)#shape is (K,3,1)
    d_sf9_dr=torch.cat(cal_d_sf9_dr(r), dim=1)#shape is (K,3,1)
    d_sf_dr=[d_sf0_dr, d_sf1_dr, d_sf2_dr, d_sf3_dr, d_sf4_dr, d_sf5_dr, d_sf6_dr, d_sf7_dr, d_sf8_dr, d_sf9_dr]
    d_sf_dr=torch.cat(d_sf_dr, dim=2)
    d_sf_dr=d_sf_dr.view(1,K,3,10)#10: sf0 to sf9
    return d_sf_dr
#%%
def interpolate(r, h):
    #r.shape (K,3), K is the number of integration points
    #h.shape (M,10,3), M elements, 1 elemnet has 10 nodes, 1 node has a 3D position, h is x or X or u
    #hr=sum_k{sf_i(r)*h_i, k=0 to 9}
    K=r.shape[0]
    r=r.permute(1,0)#shape is (3,K)
    r=r.view(3,K,1)
    w=torch.cat([sf0(r), sf1(r), sf2(r), sf3(r), sf4(r), sf5(r), sf6(r), sf7(r), sf8(r), sf9(r)], dim=1)
    w=w.view(1,K,10,1)
    M=h.shape[0]
    h=h.view(M,1,10,3)
...
def cal_dh_dr(r, h, d_sf_dr=None, numerator_layout=True):
    ...
    #r.shape (K,3), K is the number of integration points
    #h.shape (M,10,3), M elements, 1 elemnet has 10 nodes, 1 node has a 3D position
    #d_sf_dr.shape (1,K,3,10)
    M=h.shape[0]
    h=h.view(M,1,10,3)
...
def cal_d_sf_dh(r, h):
    ...
    d_sf_dr=cal_d_sf_dr(r)
    #d_sf_dr.shape (1,K,3,10), K is the number of integration points
...
def cal_F_tensor_with_d_sf_dX(x, d_sf_dX):
    #x.shape is (M,10,3)
    #d_sf_dX.shape is (M,K,3,10), from cal_d_sf_dh
    M=x.shape[0]
    K=d_sf_dX.shape[1]
    x=x.view(M,1,10,3,1).expand(M,K,10,3,1)
    d_sf_dX=d_sf_dX.transpose(2,3)#shape is (M,K,10,3)
    d_sf_dX=d_sf_dX.view(M,K,10,1,3)
    F=matmul(x, d_sf_dX).sum(dim=2)
    return F #(M,K,3,3)
#%% use 1 integration point
def cal_nodal_force_from_cauchy_stress_1i(S, d_sf_dx, det_dx_dr):
    #r=get_integration_point_1i(dtyep, devide), r.shape is (K,3), K=1
    ...
    #d_sf_dx.shape is (M,K,3,10), det_dx_dr.shape is (M,K,1,1)
    #det_dx_dr should be > 0
    #----------------------------------------------------------------------
    #force_i=integration(S*dNi_dx at r), i=0 to 9, #(9.15b) in the book
...
def cal_nodal_force_from_cauchy_stress_4i(S, d_sf_dx, det_dx_dr):
    ...
    force=(1/24)*(matmul(S, d_sf_dx)*det_dx_dr).sum(dim=1)#force.shape is (M,3,10)
    force=force.permute(0,2,1)
    force=force.contiguous()
    return force #(M,10,3)
#%% use 8 integration points
def cal_nodal_force_from_cauchy_stress_8i(S, d_sf_dx, det_dx_dr):
    #r=get_integration_point_4i(dtype, device), r.shape is (K,3), K=8
    #F=cal_F_tensor(...) on the integration point, F.shape is (M,K,3,3)
    #S=cal_Cauchy_stress(F, Mat) on the integration point, S.shape is (M,K,3,3)
    #d_sf_dx, dx_dr, det_dx_dr=cal_d_sf_dh(r, x)
    #d_sf_dx.shape is (M,K,3,10), det_dx_dr.shape is (M,K,1,1)
    ...
    #force_i=integration(S*dNi_dx at r), i=0 to 9, #(9.15b) in the book
    #shape_function_weight is not used for stress-force calculation
    w=get_integration_weight_8i(dtype=S.dtype, device=S.device)
    w=w.view(1,8,1,1)
    force=(w*matmul(S, d_sf_dx)*det_dx_dr).sum(dim=1)#force.shape is (M,3,10)
    force=force.permute(0,2,1)
    force=force.contiguous()
    return force #(M,10,3)
#%%
def cal_nodal_force_from_cauchy_stress(S, d_sf_dx, det_dx_dr):
    if det_dx_dr.shape[1] == 1:
        ...
        raise ValueError("only support 1i, 4i, and 8i")
#%% use 1 integration point
def cal_nodal_force_from_2pk_stress_1i(F, S, d_sf_dX, det_dX_dr):
    #r=get_integration_point_1i(dype, device), r.shape is (K,3), K=1
    #F=cal_F_tensor(...) on the integration point, F.shape is (M,K,3,3)
    #S=cal_Cauchy_stress(F, Mat) on the integration point, S.shape is (M,K,3,3)
    #d_sf_dx, dx_dr, det_dx_dr=cal_d_sf_dh(r, x)
    #d_sf_dx.shape is (M,K,3,10), det_dx_dr.shape is (M,K,1,1)
    #det_dx_dr should be > 0
    #----------------------------------------------------------------
    #force_i=integration(F*S*dNi_dx at r), i=0 to 9, #(page264) in the book
    #shape_function_weight is not used for stress-force calculation
    #integration_weight is 1/6 at the integration point
...
def cal_nodal_force_from_2pk_stress_4i(F, S, d_sf_dX, det_dX_dr):
    ...
    #det_dx_dr should be > 0
    #---------------------------------------------------------------
    #force_i=integration(F*S*dNi_dx at r), i=0 to 9, #(9.15b) in the book
    #shape_function_weight is not used for stress-force calculation
    #integration_weight is 1/24 at every integration point
...
def cal_nodal_force_from_2pk_stress_8i(F, S, d_sf_dX, det_dX_dr):
    ...
    w=get_integration_weight_8i(dtype=S.dtype, device=S.device)
    w=w.view(1,8,1,1)
    force=(w*matmul(matmul(F, S), d_sf_dX)*det_dX_dr).sum(dim=1)
    force=force.permute(0,2,1)
    force=force.contiguous()
    return force #(M,10,3)
#%%
...
def cal_nodal_force_from_1pk_stress_8i(S, d_sf_dX, det_dX_dr):
    ...
    w=get_integration_weight_8i(dtype=S.dtype, device=S.device)
    w=w.view(1,8,1,1)
    force=(w*matmul(S, d_sf_dX)*det_dX_dr).sum(dim=1)
    force=force.permute(0,2,1)
    force=force.contiguous()
    return force #(M,10,3)
#%%
...
def cal_strain_energy_8i(W, det_dX_dr, reduction):
    ...
    #W=cal_strain_energy_density(F, Mat) on the 4 integration points
    #energy=integration(W at r)
    a=get_integration_weight_8i(dtype=W.dtype, device=W.device)
    a=a.view(1,8)
    det_dX_dr=det_dX_dr.view(W.shape)
    energy=(a*W*det_dX_dr).sum(dim=1, keepdim=True)
    #shape is (M,1)
    if reduction is None or reduction == "none":
        pass
    elif reduction == "sum":
...
def cal_strain_energy(W, det_dX_dr, reduction):
    if det_dX_dr.shape[1]==1:
        ...
        raise ValueError("only support 1i, 4i, and 8i; det_dX_dr.shape[1]="+str(det_dX_dr.shape[1]))
